...
namespace project_hotel.Implementation.Validators
{
    public class CreateApartmentValidator : AbstractValidator<CreateApartmentDto>
    {
        ...
        public CreateApartmentValidator(HotelContext context)
        {
            _context = context;
            ...
            RuleFor(x => x.Name)
                ...
                .Must(x => !_context.Apartments.Where(y => y.Name == x).Any()).WithMessage($"There is already apartment with that name.")
                .MinimumLength(3).WithMessage("Minimum number of characters for Name is 3")
                .MaximumLength(30).WithMessage("Maximum number of characters for Name is 30");
